...
/* struct containing all elements of a UArray2b_T */
struct T {
        int height;
        int width;
        int size; 
...
T UArray2b_new(int width, int height, int size, int blocksize)
{
        ...
        /* assertions to ensure UArray2b exists with realistic dimensions */
        assert(width >= 0 && height >= 0); // ?? assert size and blocksize?
        T uarray2b;
        NEW(uarray2b);
